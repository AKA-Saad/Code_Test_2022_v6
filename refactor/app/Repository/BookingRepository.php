...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($user && $user->is('customer')) {
            $jobs = $user->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback')->whereIn('status', ['pending', 'assigned', 'started'])->latest('due')->get();
            $usertype = 'customer';
        } elseif ($user && $user->is('translator')) {
            $jobs = Job::getTranslatorJobs($user->id, 'new')->pluck('jobs');
            $usertype = 'translator';
        }
        if ($jobs) {
            // Chunk method for heavy data
            $this->chunk(500, function ($jobs) {
                foreach ($jobs as $job) {
    ...
    //  Add here Refactor here

    public function getUsersJobsHistory($data)
    {
        $user_id = $data->get('user_id');
        $pagenum = isset($data->get('page')) ? $data->get('page') : "1";
        $user = User::find($user_id);
        $usertype = '';
        $emergencyJobs = [];
        $normalJobs  = [];
        ...
        if ($user && $user->is('customer')) {
            $jobs = $user->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->latest('due')->paginate(15);
            $usertype = 'customer';
            return ['emergencyJobs' => $emergencyJobs, 'normalJobs ' => [], 'jobs' => $jobs, 'cuser' => $user, 'usertype' => $usertype, 'numpages' => 0, 'pagenum' => 0];
        } elseif ($user && $user->is('translator')) {
            ...
            return ['emergencyJobs' => $emergencyJobs, 'normalJobs ' => $normalJobs, 'jobs' => $jobs, 'cuser' => $user, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
        }
    }
    // End here
    ...
    public function AvoidRedundant($result, $message, $fieldname = '')
    {
    ...
    public function certified($data)
    {
        ...
        } else if (in_array('normal', $data['job_for']) && in_array('certified', $data['job_for'])) {
            $data['certified'] = 'both';
    ...
    public function consumertype($consumer_type)
    {
    ...
    public function store($data)
    {
        ...
        if ($user->user_type == config('USER.CUSTOMER_ROLE_ID')) {
            $cuser = $user;

            if (!isset($data['from_language_id'])) {
                // Avoid redundant data 
                return $this->AvoidRedundant('fail', "Du måste fylla in alla fält", "from_language_id");
            }
            if ($data['immediate'] == 'no') {
                if (isset($data['due_date']) && $data['due_date'] == '') {
                    return $this->AvoidRedundant('fail', "Du måste fylla in alla fält", "due_date");
                }
                ...
                if (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])) {
                    ...
                    return $this->AvoidRedundant('fail', "Du måste göra ett val här", "customer_phone_type");
                }
                if (isset($data['duration']) && $data['duration'] == '') {

                    return $this->AvoidRedundant('fail', "Du måste fylla in alla fält", "duration");
                }
            } else {
                if (isset($data['duration']) && $data['duration'] == '') {
                    return $this->AvoidRedundant('fail', "Du måste fylla in alla fält", "duration");
            ...
            if ($data['immediate'] == 'yes') {
                ...
                if ($due_carbon->isPast()) {
                    return $this->AvoidRedundant('fail', "Can't create booking in past");
                }
            }
            $data['gender'] = in_array('male', $data['job_for']) ?   'male' :  'female';

            $data = $this->certified($data);
            $data['job_type']  = $this->consumertype($consumer_type);
            ...
            return $this->AvoidRedundant('fail', "Translator can not create booking");
        }
    ...
    public function storeJobEmail($data)
    {
        ...
        $this->sendmail($user, $job, $subject, $email, 'emails.session-ended');
    ...
    public function storeJobEmail($data)
    {
        ...
        $subject = 'Vi har mottagit er tolkbokning. Bokningsnr: #' . $job->id;
        $this->sendmail($user, $job, $subject, $email,  'emails.job-created');
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $user = $job->user()->first(); //No need for get() here
        ...
        $this->sendmail($user, $job, $subject, $email,  'emails.session-ended', 'faktura', $session_time);
        ...
        $email = $user->email;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $this->sendmail($user, $job, $subject, $email,  'emails.session-ended', 'lön', $session_time);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $languages = UserLanguages::where('user_id', $user_id)->get(); // "=" bad practice
        $userlanguage = collect($languages)->pluck('lang_id'); //No need for all function
        ...
        foreach ($job_ids as $k => $job_id)   //  Naming convention bad
        {
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $message = 'Ny akutbokning för ' . $data['language'] . 'tolk ' . $data['duration'] . 'min'; // redundent and complexity
        $msg_contents = $data['immediate'] == 'no' ? $message . $data['due'] : $message;
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        foreach ($translators as $translator) {
            ...
            SendSMSHelper::send(config('SMS.SMS_NUMBER'), $translator->mobile, $message); // Don't use env direct first to config
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        // * use GuzzleHttp\Client rather than curl * //

        $headers = [
            'Authorization' => 'Bearer ' . $onesignalRestAuthKey,
            'Content-Type' => 'application/json',
        ];
        $request = new Request(
            'POST',
            config('URL'), // Avoid Hard coded URL
            $headers,
            $fields,
            true
        );
        ...
        $http = new Client();
        try {
            $response = $http->send($request, ['timeout' => 8]);
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job ' . $job_id, [$users, $data, $msg_text, $is_need_delay]);
        if (config('APP_ENV') == 'prod') { // Dont use env direct
            $onesignalAppID = config('app.prodOnesignalAppID');
            $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('app.prodOnesignalApiKey'));
        } else {
            $onesignalAppID = config('app.devOnesignalAppID');
